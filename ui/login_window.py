...
C_RED = "#B91C1C";  C_BG = "#F5F5F5";  C_FIELD = "#EDEDED"
C_TEXT = "#1C1C1C"; C_GREY = "#6B7280"
...
class _RedPanel(QWidget):
    def paintEvent(self, e):
        ...
        path.lineTo(r.left(), r.bottom()); path.closeSubpath()
...
class _Field(QFrame):
    def __init__(self, ph, echo=QLineEdit.EchoMode.Normal):
        ...
        self.setStyleSheet(f"QFrame{{background:{C_FIELD};border-radius:10px;border:1.5px solid transparent;}}")
...
class LoginPage(QWidget):
    ...
    def _build(self):
        ...
        h = QLabel("Hello,\nWelcome"); h.setFont(QFont("Segoe UI",34,QFont.Weight.Bold))
        h.setStyleSheet("color:white;background:transparent;"); ll.addWidget(h)
        s = QLabel("Don't Have an Account ?"); s.setFont(QFont("Segoe UI",12,QFont.Weight.DemiBold))
        s.setStyleSheet("color:rgba(255,255,255,0.82);background:transparent;"); ll.addWidget(s)
        ...
        reg.setStyleSheet("QPushButton{background:transparent;color:white;border:2px solid white;border-radius:10px;}"
                          "QPushButton:hover{background:rgba(255,255,255,0.15);}")
        reg.clicked.connect(lambda: self.shell.navigate("register")); ll.addWidget(reg)
        root.addWidget(left)
        ...
        if os.path.exists(logo_path):
            lbl = QLabel()
            lbl.setPixmap(QPixmap(logo_path).scaledToWidth(190, Qt.TransformationMode.SmoothTransformation))
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter); lbl.setStyleSheet("background:transparent;")
            rl.addWidget(lbl); rl.addSpacing(6)
        t = QLabel("Login"); t.setFont(QFont("Segoe UI",28,QFont.Weight.Bold))
        t.setStyleSheet(f"color:{C_RED};background:transparent;")
        t.setAlignment(Qt.AlignmentFlag.AlignCenter); rl.addWidget(t); rl.addSpacing(28)
        self._u = _Field("Username"); self._p = _Field("Password", QLineEdit.EchoMode.Password)
        self._p.inp.returnPressed.connect(self._login)
        for f in [self._u, self._p]: rl.addWidget(f); rl.addSpacing(14)
        fgt = QLabel("Forgot Password ?"); fgt.setAlignment(Qt.AlignmentFlag.AlignRight)
        fgt.setStyleSheet(f"color:{C_GREY};font-size:12px;background:transparent;"); rl.addWidget(fgt); rl.addSpacing(22)
        btn = QPushButton("Login"); btn.setFixedHeight(52)
        btn.setFont(QFont("Segoe UI",14,QFont.Weight.Bold))
        btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        btn.setStyleSheet(f"QPushButton{{background:transparent;color:{C_RED};border:2px solid {C_RED};border-radius:10px;}}"
                          f"QPushButton:hover{{background:{C_RED};color:white;}}")
        btn.clicked.connect(self._login); _shadow(btn,14,(185,28,28,50),0,3); rl.addWidget(btn)
        root.addWidget(right, stretch=1)

    def _login(self):
        user = self._u.text().strip(); pwd = self._p.text().strip()
        if not user or not pwd:
            QMessageBox.warning(self, "Required", "Please enter username and password."); return
        sess = get_session()
        u = sess.query(User).filter(User.username==user, User.is_deleted!=True).first()
        sess.close()
        if u and u.password == pwd:
            UserSession.start(u); self.shell.navigate("dashboard")
        else:
